...
import { api, type Party, type Sale } from "../lib/api";
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-600">Invoices, newest first.</p>
        </div>
      ...
      <section className="space-y-4 rounded-xl border border-neutral-200 p-4">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-neutral-700">
            Customer <span className="font-normal text-neutral-500">(optional)</span>
          </label>
      ...
      <div className="rounded-xl border border-neutral-200">
        ...
        {!isLoading && sales.length === 0 && (
          <p className="px-4 py-6 text-center text-sm text-neutral-500">No sales in this period.</p>
        )}
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="truncate font-medium text-neutral-900">
                        {partyMap.get(sale.party_id)?.name ?? `Party #${sale.party_id}`}
                      </p>
                      {cancelled && (
                    ...
                    <p className="truncate text-sm text-neutral-600">
                      {sale.invoice_no} · {formatDisplayDate(sale.invoice_date)}
                    </p>
                  </div>
                  <span
