...
import { ApiError, api, TXN_TYPE_LABELS, type BankLedgerRow, type CashLedger } from "../lib/api";
import { downloadCashLedgerPdf } from "../lib/cashLedgerPdf";
...
type CashOpening = { amount: string | null; as_of_date: string | null };
...
/** Admin-only, single field. Re-saving replaces the one opening row. */
function CashOpeningBalance() {
  const queryClient = useQueryClient();
  const { data } = useQuery({
    queryKey: ["cash-ledger", "opening-balance"],
    queryFn: () => api.get<CashOpening>("/cash-ledger/opening-balance"),
    staleTime: 0,
  });
  ...
  const shownAmount = amount ?? data?.amount ?? "";
  const shownDate = asOf ?? data?.as_of_date ?? todayIso();
  const valid = /^-?\d+(\.\d{1,2})?$/.test(shownAmount.trim()) && shownDate !== "";
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await api.putJson<CashOpening>("/cash-ledger/opening-balance", {
        as_of_date: shownDate,
        amount: shownAmount.trim(),
      });
      setAmount(null);
      setAsOf(null);
      setMessage("Opening balance saved.");
      await queryClient.invalidateQueries({ queryKey: ["cash-ledger", "opening-balance"] });
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "Could not save the opening balance.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <section className="space-y-3 rounded-xl border border-neutral-200 p-4">
      <p className="text-sm font-medium text-neutral-700">Cash opening balance (admin)</p>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label htmlFor="cash-opening-amount" className="mb-1.5 block text-xs text-neutral-500">
            Amount
          </label>
          <input
        ...
        <div>
          <label htmlFor="cash-opening-date" className="mb-1.5 block text-xs text-neutral-500">
            As of
          </label>
          <input
      ...
      <button type="button" className="btn-secondary" disabled={!valid || saving} onClick={save}>
...
export default function CashLedgerScreen() {
  ...
  const [run, setRun] = useState<{ range: DateRange; nonce: number } | null>(null);

  const { data, isPending, isFetching, isError, error } = useQuery({
    queryKey: ["cash-ledger", run?.range.from, run?.range.to, run?.nonce],
    queryFn: () => api.get<CashLedger>(`/cash-ledger?from=${run!.range.from}&to=${run!.range.to}`),
    enabled: run !== null,
    gcTime: 0,
    staleTime: 0,
  });
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-neutral-900">Cash ledger</h2>
        <p className="text-sm text-neutral-600">Cash book over a period.</p>
      </div>

      {isAdmin && <CashOpeningBalance />}
      ...
      <section className="space-y-4 rounded-xl border border-neutral-200 p-4">
        ...
        <button
          ...
          disabled={!validRange || isFetching}
          onClick={() => setRun({ range, nonce: Date.now() })}
        >
      ...
      {run === null && (
        <p className="rounded-xl border border-dashed border-neutral-300 px-4 py-8 text-center text-sm text-neutral-500">
          Pick a period above, then tap Go.
        </p>
      )}
      ...
      {isError && (
        <p role="alert" className="rounded-lg border border-danger/30 bg-danger/5 px-3 py-2 text-sm text-danger">
          {error instanceof Error ? error.message : "Could not load the cash ledger."}
        </p>
      )}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              onClick={async () => {
                ...
                setExporting(true);
                try {
                  await downloadCashLedgerPdf(data);
                } catch {
